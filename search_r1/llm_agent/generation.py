...
class LLMGenerationManager:
    ...
    def _postprocess_responses(self, responses: torch.Tensor) -> torch.Tensor:
        ...
        if self.config.no_think_rl:
            ...
            responses_str = [
                f"<answer>{envs[idx].ACTION_LOOKUP[action]}</answer>"
    ...
    def run_llm_loop(
        self, gen_batch, initial_input_ids: torch.Tensor
    ) -> Tuple[Dict, Dict]:
    ...
    def _compose_final_output(
        self, left_side: Dict, right_side: Dict, meta_info: Dict
    ) -> Tuple[Dict, Dict]:
    ...
    def execute_predictions(
        ...
        if do_search:
            ...
            assert len(search_results) == sum(
                [1 for action in cur_actions if action == "search"]
            )
        else:
            search_results = [""] * sum(
                [1 for action in cur_actions if action == "search"]
