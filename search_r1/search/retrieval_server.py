...
class Encoder:
    ...
    def encode(self, query_list: List[str], is_query=True) -> np.ndarray:
        ...
        if t_total > 0.05:  # 只有当编码时间显著时才打印
            ...
            print(
                f"  --> Tokenize: {t_tok*1000:.2f}ms | ToGPU: {t_cuda*1000:.2f}ms | Inference: {t_infer*1000:.2f}ms | ToCPU: {t_cpu*1000:.2f}ms"
            )
...
class DenseRetriever(BaseRetriever):
    def __init__(self, config):
        ...
        if not config.faiss_gpu:
            ...
            print(
                f"[Config] CPU Mode detected. Force FAISS OMP threads to: {faiss.omp_get_max_threads()}"
            )
        # ==================================
    ...
    def batch_search(
        ...
        # 阶段耗时统计
        time_stats = {"encode": 0.0, "search": 0.0, "load": 0.0}
