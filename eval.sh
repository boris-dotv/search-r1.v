...
# 定义三个环境的 Python 绝对路径 (确保这些环境都存在)
PY_RETRIEVER="$ENVS_DIR/retriever/bin/python" # 必须有: torch, transformers, flask/fastapi
PY_SGL="$ENVS_DIR/sgl/bin/python"             # 必须有: sglang
PY_EVAL="$ENVS_DIR/searchr1/bin/python"       # 必须有: pandas, openai, aiohttp, tqdm

# 项目路径
PROJECT_ROOT="/public_hw/share/cit_ztyu/cz/Search-R1"
MODEL_PATH="$PROJECT_ROOT/verl_checkpoints/nq-search-r1-ppo-qwen2.5-3b-instruct-em/actor/global_step_150"
TEST_DATA="$PROJECT_ROOT/data/nq_search/test.parquet"
...
# 端口配置
PORT_SGLANG=$(shuf -i 30000-40000 -n 1)
PORT_RETRIEVER=$(shuf -i 40001-50000 -n 1)

# 并发控制 (16 是最稳妥的，既快又不会打挂服务)
CONCURRENCY=16
...
# 自动清理后台进程
trap 'pkill -P $$; echo ">> [Cleanup] 已清理所有后台服务"; exit' EXIT SIGINT SIGTERM

echo "========================================================"
echo ">> 任务启动: Search-R1 评估 (Full Replication Mode)"
echo ">> SGLang Port: $PORT_SGLANG | Retriever Port: $PORT_RETRIEVER"
echo "========================================================"
...
nohup $PY_RETRIEVER search_r1/search/retrieval_server.py \
    ...
    --port $PORT_RETRIEVER \
    --topk 3 > retriever_eval.log 2>&1 &

echo ">> 等待检索服务就绪..."
for i in {1..60}; do
    # 只要端口通了就算成功
    if curl -s "http://127.0.0.1:$PORT_RETRIEVER/retrieve" > /dev/null || \
       curl -o /dev/null -s -w "%{http_code}\n" "http://127.0.0.1:$PORT_RETRIEVER/retrieve" | grep -q "405"; then
        echo ">> Retriever 已就绪!"
        break
    fi
    sleep 1
...
# 2.2 启动 SGLang 推理服务 (GPU 0)
echo ">> [GPU 0] 正在启动 SGLang 推理服务..."
export CUDA_VISIBLE_DEVICES=0
nohup $PY_SGL -m sglang.launch_server \
    --model-path "$MODEL_PATH" \
    --host 0.0.0.0 \
    --port $PORT_SGLANG \
    --log-level error \
    --tp 1 > sglang_eval.log 2>&1 &

echo ">> 等待 SGLang 加载模型 (可能需要 1-2 分钟)..."
for i in {1..300}; do
    if curl -s "http://127.0.0.1:$PORT_SGLANG/v1/models" | grep "id" > /dev/null; then
        echo ">> SGLang 已就绪!"
        break
    fi
    sleep 2
...
# ================= 3. 执行评估 =================
echo ">> 开始高并发评估 (Concurrency: $CONCURRENCY)..."
$PY_EVAL eval_script.py \
    --sglang_url "http://127.0.0.1:$PORT_SGLANG" \
    --retriever_url "http://127.0.0.1:$PORT_RETRIEVER/retrieve" \
    --data_path "$TEST_DATA" \
