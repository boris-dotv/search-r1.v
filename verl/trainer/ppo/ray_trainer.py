...
class RayPPOTrainer(object):
    ...
    def _save_checkpoint(self):
        ...
        if self.use_critic:
            ...
            # =======================================================
            # 【核心修复】: 在保存 Critic 之前强制清理内存
            # 解决 vLLM 内存泄漏导致 Actor 刚存完，Critic 没内存存的问题
            # =======================================================
            import gc
            import torch
            import time

            print("[CheckPoint] Force collecting garbage (CPU)...")
            gc.collect()  # 1. 强制回收 Python 对象引用

            print("[CheckPoint] Emptying CUDA cache...")
            torch.cuda.empty_cache()  # 2. 尝试释放显存碎片（虽然主要是内存问题，但有备无患）

            print("[CheckPoint] Waiting 10s for OS memory release...")
            time.sleep(10)  # 3. 关键：给操作系统时间回收 Buffer，给 Critic 腾出空间
            # =======================================================
