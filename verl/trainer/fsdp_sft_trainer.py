    ...
    def _compute_loss(self, batch):
        ...
        if self.config.data.balance_dp_token:
            torch.distributed.all_reduce(
                valid_token_this_rank
            )  # becomes total valid tokens in all ranks
            dp_size = torch.distributed.get_world_size()
        else:
            dp_size = 1

        loss = (
            torch.sum(loss) / valid_token_this_rank * dp_size
        )  # possible bugs here for dp
        return loss
    ...
    def fit(self):
        ...
        if self.config.trainer.validate_before_training:
            ...
            for data in self.val_dataloader:
                data = TensorDict(
                    data, batch_size=self.config.data.micro_batch_size
                ).cuda()
            ...
            for data in self.train_dataloader:
                ...
                if global_step >= self.total_training_steps:
                    ...
                    for val_data in self.val_dataloader:
                        val_data = TensorDict(
                            val_data, batch_size=self.config.data.micro_batch_size
                        ).cuda()
...
def main(config):
    ...
    device_mesh = init_device_mesh(
        device_type="cuda", mesh_shape=(world_size,), mesh_dim_names=("dp",)
    )
