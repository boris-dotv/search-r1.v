...
class ActorRolloutRefWorker(MegatronWorker):
    ...
    def __init__(self, config: DictConfig, role: str):
        ...
        if self._is_actor and self._is_rollout:
            ...
            self._is_offload_optimizer = self.config.actor.get(
    ...
    def _build_model_optimizer(
        ...
        def megatron_actor_model_provider(pre_process, post_process):
            ...
            )  # this will be set inside get_model
    ...
    def _build_rollout(self):
        if self.config.rollout.name == "vllm":
            ...
            layer_name_mapping = {
                "qkv_layer_name": self.config.rollout.layer_name_map.get(
                    "qkv_layer_name", "qkv"
                ),
                "gate_proj_layer_name": self.config.rollout.layer_name_map.get(
            ...
            params = normalize_pp_vpp_params(
                params=params,
    ...
    def init_model(self):
        ...
        megatron_config = OmegaConf.create(
            {
                "sequence_parallel": self.config.actor.megatron.get(
    ...
    def compute_ref_log_prob(self, data: DataProto):
        ...
        if self._is_offload_param:
            load_megatron_param_and_grad(
                self.ref_module, torch.cuda.current_device(), self._is_offload_grad
            )
...
class CriticWorker(MegatronWorker):
    ...
    def _build_critic_model_optimizer(
        ...
        override_config_kwargs.update(override_model_config)
        update_model_config(
            critic_model_config, override_config_kwargs=override_config_kwargs
        )
    ...
    def init_model(self):
        ...
        megatron_config = OmegaConf.create(
            {
                "sequence_parallel": self.config.megatron.get(
                    "sequence_parallel", True
                ),
                "param_dtype": PrecisionType.to_str(torch_dtype),
                "tensor_model_parallel_size": mpu.get_tensor_model_parallel_world_size(),
...
class RewardModelWorker(MegatronWorker):
    ...
    def _build_rm_model(
        ...
        override_config_kwargs.update(override_model_config)
        update_model_config(
            rm_model_config, override_config_kwargs=override_config_kwargs
        )
    ...
    def init_model(self):
        ...
        sft_tokenizer_local_path = copy_local_path_from_hdfs(
