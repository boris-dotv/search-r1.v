...
class MegatronPPOActor(BasePPOActor):
        ...
        if recompute_old_log_prob or "old_log_probs" not in data.batch.keys():
            ...
            with torch.no_grad():
                output = self.forward_backward_batch(
                    data, forward_only=True, post_process_fn=compute_logprobs_fn
                )
                if mpu.is_pipeline_last_stage(ignore_virtual=True):
                    # only on last rank. It should be on every tp rank
                    log_probs = torch.cat(
                        [o["log_probs"] for o in output], dim=0
                    )  # (bs, seq_size)
    ...
    def forward_backward_batch(
        self, data: DataProto, forward_only=False, post_process_fn=None
    ):
        """
        We assume:
        ...
        for data in dataloader:
            ...
            for chunk in self.actor_module:
                ...
                chunk.zero_grad_buffer(
                    zero_buffer=(
                        not self.actor_optimizer_config.use_distributed_optimizer
                    )
                )
