...
class MegatronPPOCritic(BasePPOCritic):
    ...
    def __init__(
        ...
        if self.config.kl_ctrl.type == "fixed":
            ...
            assert (
                self.config.kl_ctrl.horizon > 0
            ), f"horizon must be larger than 0. Got {self.config.kl_ctrl.horizon}"
    ...
    def compute_values(self, data: DataProto) -> DataProto:
        ...
        with torch.no_grad():
            ...
            if mpu.is_pipeline_last_stage(ignore_virtual=True):
                ...
                values = torch.cat(
                    [o["vpreds"] for o in output], dim=0
    ...
    def forward_backward_batch(self, data: DataProto, forward_only=False):
        ...
        batches = split_dict_tensor_into_batches(
            data.batch, batch_size=self.config.ppo_micro_batch_size
        )
        ...
        batch_generator = make_batch_generator(
            batches, vpp_size=len(self.critic_module)
        )
        ...
        if mpu.get_pipeline_model_parallel_world_size() > 1:
            losses_reduced = forward_backward_func(
                ...
                seq_length=self.config.ppo_micro_batch_size
        ...
        for data in dataloader:
            ...
            for chunk in self.critic_module:
                chunk.zero_grad_buffer(
                    zero_buffer=(
                        not self.critic_optimizer_config.use_distributed_optimizer
                    )
                )
