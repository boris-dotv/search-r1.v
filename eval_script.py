                ...
                if match:
                    ...
                    if action == "search":
                        ...
                        results = await search_query(
                            search_session, retriever_url, inner_content
                        )
                        # 格式化并插入 Observation
...
async def main():
    ...
    # 设置 Client 超时
    client = AsyncOpenAI(
        base_url=f"{args.sglang_url}/v1", api_key="EMPTY", timeout=600.0
    )
    ...
    async with aiohttp.ClientSession(connector=connector) as search_session:
        tasks = [
            solve_one_question(row, client, search_session, args.retriever_url, sem)
            for row in records
        ]
